...
Test clips, the standard matrix, and the rule for what lives in the repo versus
`$REEL_TESTING_DIR` (default `~/testing`) are documented in `docs/PERF_CORPUS.md`;
the clip matrix manifest is `scripts/perf/clips.tsv`.
