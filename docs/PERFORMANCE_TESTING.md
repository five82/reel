...
| TQ probes | Below UHD: 3 x 48-frame windows; 5 windows after high spread; full-chunk at <=256 frames; full-first reuse at <=720 median-seeded frames. **UHD (>=3840px): full-scan -- every probe is a whole-chunk encode** (resolution-aware threshold == largest chunk via `TargetQualityFullProbeFrames`). | Below UHD is a good speed/accuracy tradeoff (GPU-bound). UHD went full-scan 2026-06-29: strictly more accurate (fullvalidate gap 0 by construction), no measured wall regression (UHD is encoder-bound), removes the window-spread->monotonicity mechanism. | 2026-06-13 knobs; 2026-06-18 rebaseline; 2026-06-29 full-scan A/B + switch |
...
| Probe-sample noise vs sample-frame count | Structurally gated because current-band clips converge well. Revisit only if multiple current-band clips show recurring probe tails or sampled-vs-full gaps; use `sullyhv-15m-4k-hdr` plus an easy control and `fullvalidate`. **2026-06-29:** the feature tail is genuine within-shot variance, not sampling noise -- the feature's already-full-probed <=256 band still hits 9% max_probe / 13% monotonicity. Variable window count is the wrong direction for any over-encoding (see next row). |
| Sampled-vs-full proxy bias (over-encoding clean content) | **Corrected 2026-06-28 (was elevated on an artifact):** the prior +0.585 over-encoding measurement was a fullvalidate artifact -- rebaseline-20260617 workdirs had their sources re-cut to cropped dims after encode, so scoring the re-cut source against original-source IVFs inflated scores by ~+0.5. Fresh encodes (source==IVF throughout) show reel is WELL-CALIBRATED: real mean +0.02 to +0.085 over the 9.35 target, in band, across air (1080p clean-light), bts (1080p heavy), sully-5m (4K clean), sullyhv (4K hard). The standalone-window-pessimism mechanism story was also refuted (full-chunk-probe chunks showed the same artifact; fresh ones show production==fullvalidate gap 0). So the bias is real but SMALL (~+0.08, in band), NOT a +0.6 size prize. Back to low priority; do NOT pursue a proxy-target offset or per-chunk bias model on the basis of the artifact. Methodology: a kept workdir is only valid for fullvalidate if the source is unchanged since encode. See the 2026-06-28 scope-check entry (and the CONTAMINATED over-encoding entry it corrects). |
...
| **UHD full-scan probes (SHIPPED 2026-06-29)** | Implemented: `encode.TargetQualityFullProbeFrames` makes every UHD (>=3840px) probe a whole-chunk encode (threshold == largest chunk); below UHD keeps 3x48 sampling. Verified on UHD (sully-5m: single-window probes, 435s) and 1080p (im: 3-window probes preserved, 150s). Accuracy case is structural and content-independent (fullvalidate gap 0.0000 by construction, over-encodes ~0.05-0.065 JOD less, 0 below floor). Wall shows no measured UHD regression (n=1, deltas bidirectional within noise); the 1080p leg confirmed why sampling stays there (full-scan +18.7%, GPU-bound). Caveat: the motivating feature tail was never directly reproduced (sullyhv has no tail; 5m clips too short) -- the accuracy/structural wins hold without it. See the 2026-06-29 full-scan LOG entry (with fresh-review caveats). |
...
| Pooling/sharing a `video.Source` across probe windows | Rejected. Per-window `video.Open`/`Probe` overhead is ~75 ms (4K) / ~39 ms (1080p) gross, but ~90% overlap-hidden by the probe/score pipeline; only `encodeProbe`'s serial `Open(SOURCE)` is on the critical path (~0.3% of 4K feature wall, ~0% on GPU-bound 1080p). `Source` is not concurrency-safe, so pooling adds thread-safety complexity for a sub-1% win. See the 2026-06-28 probe-open entry. |
| Generalizing full-first probe to neighbor-seeded chunks | **Not rejected (corrected 2026-06-28).** The original rejection rested on a winrescore "100% flip / +0.66 gap" that was the re-cut-source artifact (same as the over-encoding entry): it scored rebaseline final IVFs against re-cut sources, inflating scores ~+0.5. Valid winrescore on fresh workdirs (n=6: sully clean, kbv1 grain, im 1080p) shows the mid-GOP-vs-standalone window-blend gap is +0.056 mean (within ~0.075 probe noise), flip rate 1/6 = 17%, and not directionally consistent. So full-first convergence ~= sampled; the encode-free frame-budget replay (ungated = coin-flip because save 144 ~= waste F-144 on 4K; gated F<=264 = +EV, excludes the 15 low-convergence max-duration chunks) is roughly trustworthy. full-first is a plausible-but-unverified speed lever, NOT ruled out. Left on file, not pursued (performance tuning closed); a fresh-encode A/B would be needed to confirm the gated version. See the 2026-06-28 full-first entry (corrected). |
| Decoupling probe preset from final preset (probes faster than p6) | Not promising. Probes are 82-91% of 4K encode work and look shrinkable, but archived `preset-ab` data (encode-free) shows the CRF offset p6->p8 at fixed JOD is small (~1.3 CRF) and SIGN-FLIPS by content (sully clean -1.3, kbv1/im +1.3), so no fixed correction works. Wall win is ~7-9% on 4K only (less than whole-p8's 12-13%, since the final stays p6), ~0 on GPU-bound 1080p. And the offset works against size on clean content: sully's p8 probes converge to a CRF ~1.3 LOWER, so the p6 final is ~8-10% BIGGER -- trading wall for a size increase on exactly the content the 4K win would matter for. Grainy 4K gets wall+size; 1080p gets neither. Distinct from the 2026-06-20 final-output preset sweep (keep preset 6). See the 2026-06-28 probe-preset-decoupling entry. |
