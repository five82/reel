...
typedef struct {
  ...
  int num_equations;
  double total;
} reel_aom_noise_strength_solver_t;
...
int reel_aom_noise_strength_solver_fit_piecewise(
    const reel_aom_noise_strength_solver_t *solver, int max_num_points,
...
typedef struct {
  reel_aom_noise_shape shape;
  int lag;
  int bit_depth;
  int use_highbd;
} reel_aom_noise_model_params_t;
 ...
 */
typedef struct {
  reel_aom_noise_model_params_t params;
  reel_aom_noise_state_t combined_state[3];  // Combined state per channel
  reel_aom_noise_state_t latest_state[3];    // Latest state per channel
  int (*coords)[2];  // Offsets (x,y) of the coefficient samples
  int n;             // Number of parameters (size of coords)
  int bit_depth;
} reel_aom_noise_model_t;
