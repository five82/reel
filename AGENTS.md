...
- Stored analysis artifacts (kept workdirs, `fullvalidate` JSON, replay logs) are only valid while the inputs that produced them are unchanged -- re-cutting or re-encoding a source afterward silently invalidates any workdir derived from it. Verify an artifact's preconditions before trusting it for a conclusion, and treat a surprising or alarming result as a prompt to re-check the measurement before acting on it. A single-clip finding is not a corpus result; scope across content types before committing to it. (A re-cut source scored against original-source IVFs manufactured a "+0.6 JOD over-encoding / 57% size win" that did not exist, and a one-clip A/B nearly logged it as a real lever; fresh cross-content encodes exposed the artifact. See `docs/PERFORMANCE_TESTING_LOG.md`.)
